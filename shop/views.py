...
def search(request):
    ...
    #if query does not match for products or empty 
    if len(query)<3:
        params = {'msg': "Please make sure to enter relevent search query"}
    elif len(allProds) == 0:
        params = {'msg': "Opps! Product not found"}
...
def checkout(request):
    if request.method=="POST":
        ...
        thank = True
        id = order.order_id
        #This will request Paytm to transfer the amount to your account after payment by user
